...
template<typename _Tp> requires std::convertible_to<_Tp, double>\
inline IEEE754_Dbl64 operator operation (const IEEE754_Dbl64& x, _Tp scaler)\
{\
    return IEEE754_Dbl64(x.x operation scaler);\
}\
\
template<typename _Tp> requires std::convertible_to<_Tp, double>\
inline IEEE754_Dbl64 operator operation (_Tp scaler, const IEEE754_Dbl64& x)\
{\
    return IEEE754_Dbl64(scaler operation x.x);\
}\
\
inline IEEE754_Dbl64 operator operation (const IEEE754_Dbl64& x, const IEEE754_Dbl64& y)\
{\
    return IEEE754_Dbl64(x.x operation y.x);\
}
/*
