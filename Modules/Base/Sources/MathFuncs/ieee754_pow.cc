...
{
    return __IEEE754_POW64F(_X, _Power);
}

complex64 __cdecl powc(complex64 _X, complex64 _Power, int64 _K)
{
    return expc(_Power * lnc(_X, _K));
}
...
const struct __GNU_Table_Powlnt {double invc, pad, lnc, lnctail;} __Pow64f_ln_table[]
{
...
__Float64 __cdecl __IEEE754_POW64F(__Float64 _X, __Float64 _Power)
{
    ...
    {
        return 2 * i - 1 >= 2 * uint64(POS_INF_DOUBLE) - 1;
    };
    ...
    if (topx - 0x001 >= 0x7ff - 0x001 || (topy & 0x7ff) - 0x3be >= 0x43e - 0x3be)
    {
        ...
        if (zeroinfnan(iy))
        {
            ...
            if (2 * ix > 2 * uint64(POS_INF_DOUBLE) || 2 * iy > 2 * uint64(POS_INF_DOUBLE)) {return x + y;}
        ...
        if (zeroinfnan(ix))
        {
            ...
            if (2 * ix == 0 && iy >> 63) {return sign_bias ? __Float64::FromBytes(NEG_INF_DOUBLE) : __Float64::FromBytes(POS_INF_DOUBLE);}
        ...
        if (ix >> 63)
        {
            ...
            if (yint == 0) {return __Float64::FromBytes(BIG_NAN_DOUBLE);}
        ...
        if ((topy & 0x7ff) - 0x3be >= 0x43e - 0x3be)
        {
            ...
            return (ix > __Float64(1.0).Bytes) == (topy < 0x800)
                       ? __Float64::FromBytes(POS_INF_DOUBLE)  // Overflow
                       : __Float64(0); // Underflow
        }
        if (topx == 0)
        {
    ...
    static auto ln_inline = [](uint64_t ix, __Float64* tail)
    {
        ...
        invc = __Pow64f_ln_table[i].invc;
        logc = __Pow64f_ln_table[i].lnc;
        logctail = __Pow64f_ln_table[i].lnctail;
...
#if 0 // pow test function generated by Deepseek
#include <iostream>
#include <cmath>
#include <iomanip>
#include <fstream>
...
std::vector<TestCase> generateTestCases()
{
    ...
    // 特殊值
    cases.push_back({0.0, 0.0, "0^0"});
    cases.push_back({0.0, 1.0, "0^1"});
    cases.push_back({1.0, 0.0, "1^0"});
    cases.push_back({1.0, 1.0, "1^1"});
    cases.push_back({1.0, std::numeric_limits<double>::infinity(), "1^inf"});
    cases.push_back({1.0, -std::numeric_limits<double>::infinity(), "1^-inf"});
    cases.push_back({std::numeric_limits<double>::infinity(), 0.0, "inf^0"});
    cases.push_back({std::numeric_limits<double>::infinity(), 1.0, "inf^1"});
    cases.push_back({2.0, std::numeric_limits<double>::infinity(), "2^inf"});
    cases.push_back({0.5, std::numeric_limits<double>::infinity(), "0.5^inf"});
    cases.push_back({2.0, -std::numeric_limits<double>::infinity(), "2^-inf"});
    cases.push_back({0.5, -std::numeric_limits<double>::infinity(), "0.5^-inf"});
    cases.push_back({std::numeric_limits<double>::quiet_NaN(), 1.0, "NaN^1"});
    cases.push_back({1.0, std::numeric_limits<double>::quiet_NaN(), "1^NaN"});

    // 负数处理
    cases.push_back({-2.0, 2.0, "(-2)^2"});
    cases.push_back({-2.0, 3.0, "(-2)^3"});
    cases.push_back({-2.0, 2.5, "(-2)^2.5 (should be NaN)"});
    cases.push_back({-0.5, -1.0, "(-0.5)^-1"});
    cases.push_back({-0.5, -2.0, "(-0.5)^-2"});

    // 极端值
    cases.push_back({std::numeric_limits<double>::min(), std::numeric_limits<double>::min(), "min^min"});
    cases.push_back({std::numeric_limits<double>::min(), -std::numeric_limits<double>::min(), "min^-min"});
    cases.push_back({std::numeric_limits<double>::max(), std::numeric_limits<double>::max(), "max^max"});
    cases.push_back({std::numeric_limits<double>::max(), -std::numeric_limits<double>::max(), "max^-max"});
    cases.push_back({1e-300, 1e-300, "1e-300^1e-300"});
    cases.push_back({1e300, 1e300, "1e300^1e300"});

    // 接近1的值
    cases.push_back({1.0 + std::numeric_limits<double>::epsilon(), 1.0, "(1+eps)^1"});
    cases.push_back({1.0 - std::numeric_limits<double>::epsilon(), 1.0, "(1-eps)^1"});
    cases.push_back({1.0, 1.0 + std::numeric_limits<double>::epsilon(), "1^(1+eps)"});
    cases.push_back({1.0, 1.0 - std::numeric_limits<double>::epsilon(), "1^(1-eps)"});

    // 常规值
    cases.push_back({2.0, 3.0, "2^3"});
    cases.push_back({2.0, -3.0, "2^-3"});
    cases.push_back({3.0, 2.5, "3^2.5"});
    cases.push_back({3.0, -2.5, "3^-2.5"});
    cases.push_back({10.0, 0.5, "10^0.5"});
    cases.push_back({100.0, 0.01, "100^0.01"});
    cases.push_back({M_PI, M_E, "pi^e"});
    cases.push_back({M_E, M_PI, "e^pi"});
...
int main()
{
    ...
    std::ofstream excelFile("pow_test_report.csv");

    // CSV header
    excelFile << "Description,x,y,IEEE754_POW64F Result,std::pow Result,"
              << "Absolute Difference,Relative Difference,ULP Difference\n";
    ...
    for (const auto& testCase : testCases)
    {
        ...
        // 计算结果
        double customResult = __IEEE754_POW64F(__Float64(x), __Float64(y)).x;
        double stdResult = std::pow(x, y);

        // 计算差异
        double absDiff = std::abs(customResult - stdResult);
        double relDiff = (stdResult != 0) ? absDiff / std::abs(stdResult) : 0.0;

        // 计算ULP差异
        uint64_t customBits = *reinterpret_cast<uint64_t*>(&customResult);
        uint64_t stdBits = *reinterpret_cast<uint64_t*>(&stdResult);
        int64_t ulpDiff = std::abs(static_cast<int64_t>(customBits) - static_cast<int64_t>(stdBits));

        // 写入CSV
        excelFile << std::setprecision(15)
                  << testCase.description << ","
                  << x << "," << y << ","
                  << customResult << "," << stdResult << ","
                  << absDiff << "," << relDiff << "," << ulpDiff << "\n";
    }

    excelFile.close();
    std::cout << "Results exported to pow_test_report.csv" << std::endl;
}
#endif

#if 0 // Complex pow test program generated by Deepseek
#include <iostream>
#include <complex>
#include <cmath>
#include <vector>
...
// 辅助函数：比较并打印结果
void compare_pow(complex64 x, complex64 power, int64_t k = 0)
{
    complex64 custom_result = powc(x, power, k);
    complex64 std_result = std::pow(x, power);

    std::cout << "Test case: pow(" << x << ", " << power << ")";
    if (k != 0) std::cout << " with k=" << k;
    std::cout << "\n";

    std::cout << "  Custom pow: " << custom_result << "\n";
    std::cout << "  Std   pow:  " << std_result << "\n";

    // 计算差异
    double diff_real = abs(custom_result.real() - std_result.real());
    double diff_imag = abs(custom_result.imag() - std_result.imag());

    std::cout << "  Difference: " << diff_real << " (real), "
         << diff_imag << " (imag)\n";
    std::cout << "------------------------------------------------\n";
}
...
int main()
{
    ...
    // 2. 纯虚数情况
    compare_pow(complex64(0, 1), 2.0);
    compare_pow(complex64(0, 2), complex64(0, 1));

    // 3. 一般复数情况
    compare_pow(complex64(1, 1), complex64(2, 0));
    compare_pow(complex64(1, 1), complex64(0, 1));
    compare_pow(complex64(1, 1), complex64(1, 1));

    // 4. 多值性测试 (k != 0)
    compare_pow(complex64(1, 1), complex64(0.5, 0), 0);
    compare_pow(complex64(1, 1), complex64(0.5, 0), 1);
    compare_pow(complex64(1, 1), complex64(0.5, 0), -1);

    // 5. 边界情况
    compare_pow(0.0, complex64(1, 1));
    compare_pow(complex64(1, 1), 0.0);
    compare_pow(complex64(0, 0), complex64(0, 0));

    // 6. 特殊角度
    const double pi = 3.14159265358979323846;
    compare_pow(complex64(cos(pi/4), sin(pi/4)), 2.0);
    compare_pow(complex64(cos(pi/3), sin(pi/3)), complex64(0, 1/pi));
}
#endif
