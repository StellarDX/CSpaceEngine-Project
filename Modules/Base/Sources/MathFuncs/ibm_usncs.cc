...
complex64 __cdecl __GLIBCT_SIN64C(complex64 _X)
{
    ...
    if (IClass >= FP_ZERO)
    {
        ...
        if (RClass >= FP_ZERO)
        {
            ...
            if (XReal > DBL_MIN)
            {
                sinix = __IBM_SIN64F(XReal);
                cosix = __IBM_COS64F(XReal);
            }
            else
            {
                sinix = XReal;
            ...
            if (abs(XImag) > ExpHuge)
            {
                float64 expmax = __IEEE754_EXP64F(ExpHuge);
                float64 ix = abs(XImag);
                if (std::signbit(XImag)) {cosix = -cosix;}
                ix -= ExpHuge;
                sinix *= expmax / 2;
                cosix *= expmax / 2;
                ...
                if (ix > ExpHuge)
                {
                    ...
                    return {DBL_MAX * sinix, DBL_MAX * cosix};
                }
                else
                {
                    float64 expix = __IEEE754_EXP64F(ix);
                    return {expix * sinix, expix * cosix};
                }
            }
            else
            {
                return {cosh(XImag) * sinix, sinh(XImag) * cosix};
            }
        }
        else
        {
    ...
    else if (IClass == FP_INFINITE)
    {
        ...
        else if (RClass > FP_ZERO)
        {
            ...
            float64 YReal = ::copysign(__Float64::FromBytes(POS_INF_DOUBLE), sinix);
            float64 YImag = ::copysign(__Float64::FromBytes(POS_INF_DOUBLE), cosix);

            if (Neg) {YReal = -YReal;}
            if (std::signbit(XImag)) {YImag = -YImag;}

            return {YReal, YImag};
        }
        else
        {
            return {XReal - XReal, __Float64::FromBytes(POS_INF_DOUBLE)};
        }
    }
    else
    {
...
#if 0 // Test program for complex sin generated by Deepseek.
#include <iostream>
#include <complex>
#include <cmath>
#include <iomanip>
...
// 辅助函数，用于比较两个复数是否近似相等
bool approx_equal(complex64 a, complex64 b, double rel_tol = 1e-10, double abs_tol = 1e-10)
{
    if (isnan(a.real()) || isnan(a.imag()) || isnan(b.real()) || isnan(b.imag()))
    {
        return (isnan(a.real()) == (isnan(b.real())) &&
               (isnan(a.imag())) == (isnan(b.imag())));
    }

    if (isinf(a.real()) || isinf(a.imag()) || isinf(b.real()) || isinf(b.imag()))
    {
        return (isinf(a.real()) && isinf(b.real()) && std::signbit(a.real()) == std::signbit(b.real())) &&
               (isinf(a.imag()) && isinf(b.imag()) && std::signbit(a.imag()) == std::signbit(b.imag()));
    }

    double diff_real = abs(a.real() - b.real());
    double diff_imag = abs(a.imag() - b.imag());

    double max_real = max(abs(a.real()), abs(b.real()));
    double max_imag = max(abs(a.imag()), abs(b.imag()));

    bool real_ok = diff_real <= abs_tol || diff_real <= rel_tol * max_real;
    bool imag_ok = diff_imag <= abs_tol || diff_imag <= rel_tol * max_imag;

    return real_ok && imag_ok;
}

// 打印比较结果
void print_comparison(const complex64& input, const complex64& custom_result, const complex64& std_result)
{
    std::cout << "Input: " << input << '\n';
    std::cout << "Custom sin: " << custom_result << '\n';
    std::cout << "Std   sin: " << std_result << '\n';

    if (approx_equal(custom_result, std_result))
    {
    ...
    else
    {
        ...
        std::cout << "Difference: " << (custom_result - std_result) << '\n';
    }
    std::cout << "----------------------------------" << '\n';
}
...
int main()
{
    ...
    {
        complex64 custom_result = __GLIBCT_SIN64C(z);
        complex64 std_result = sin(z);
        print_comparison(z, custom_result, std_result);
    }
}
#endif
