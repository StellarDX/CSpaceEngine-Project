...
__Float64 __cdecl __IEEE854_ASIN128F_C64F(__Float64 _X)
{
    ...
    else if (a < 0.5L)
    {
        ...
        {
            return R2D.x * x; // return x with inexact if x!=0
        }
        else
        {
            t = x * x;
...
int main() {
    ...
    compare_asin(-0.5); // -30度
    compare_asin(cse::sqrt(3.0)/2.0); // 60度
    compare_asin(-cse::sqrt(3.0)/2.0); // -60度
    compare_asin(cse::sqrt(2.0)/2.0); // 45度
    compare_asin(-cse::sqrt(2.0)/2.0); // -45度
    ...
    std::cout << "  Custom asin(NaN): " << custom_nan << " (isnan: " << isnan(custom_nan) << ")\n\n";

    std::cout << "Testing x > 1.0:\n";
    double custom_inf = __IEEE854_ASIN128F_C64F(1.1);
    std::cout << "  Custom asin(1.1): " << custom_inf << " (isnan: " << isnan(custom_inf) << ")\n\n";

    std::cout << "Testing x < -1.0:\n";
    double custom_neg_inf = __IEEE854_ASIN128F_C64F(-1.1);
    std::cout << "  Custom asin(-1.1): " << custom_neg_inf << " (isnan: " << isnan(custom_neg_inf) << ")\n";
