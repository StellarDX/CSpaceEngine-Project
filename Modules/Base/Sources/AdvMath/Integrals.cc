...
float64 __Romberg_Integral_Engine::operator()(float64 _Min, float64 _Max) const
{
    ...
    for (size_t i = 1; i < _M_MaxSteps; ++i)
    {
        ...
        if ((i > 1 && fabs(Rp[i - 1] - Rc[i]) < pow(10, -_M_p_Acc)) || (isinf(Rc[i]) || isnan(Rc[i])))
        {
            float64 Result = Rc[i];
            delete[] R1;
            delete[] R2;
...
matrix<5, 5> __Romberg_Integral_Engine::RombergAnalysis(function<float64(float64)> _Func, float64 _Min, float64 _Max)
{
    enum {hm, T, S, C, R};
    matrix<5, 5> Sequenses(0.);
...
matrix<5, 5> __Romberg_Integral_Engine::RombergAnalysis(float64 _Min, float64 _Max)
{
